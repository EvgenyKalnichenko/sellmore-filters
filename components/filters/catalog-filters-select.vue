  ...
  methods: {
    ...
    getArraySelectedFilter(data) {
      if (!this.selected.find(p => data.label === p.label)) {
        ...
        this.selected = Array.from(this.selected.filter(prod => prod.value !== data.value))
      }
    },
    handlerInput(data) {
      this.getArraySelectedFilter(data)
